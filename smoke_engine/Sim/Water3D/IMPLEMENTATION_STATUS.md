...
- Shared 3D smoke/coupled simulation path
...
- Exact feature parity with every 2D-only behavior and tuning path

## Practical meaning

Today:
- The current macOS build still runs the existing 2D CPU app.
- The separated 3D module is a real CPU MAC water solver, not just a particle prototype.
- There is now a source-only CUDA backend for the 3D solver, but it has not been compiled or run in this workspace.
