...
// -------------------- Multigrid Preconditioner (C1) --------------------

void MAC2D::ensureMultigrid() {
    if (!mgDirty) return;
    mgDirty = false;
    ...
    while ((int)mgLevels.size() < mgMaxLevels) {
        ...
        MGLevel C;
        C.nx = cnx; C.ny = cny;
        C.invDx2 = F.invDx2 * 0.25f; // dx doubles -> invDx2 / 4
        ...
        // Conservative solid: if any child solid -> solid
        for (int J = 0; J < cny; ++J) {
            for (int I = 0; I < cnx; ++I) {
                int fi = 2*I;
                int fj = 2*J;
...
void MAC2D::mgApplyA(int lev, const std::vector<float>& x, std::vector<float>& Ax) const {
...
void MAC2D::mgSmoothJacobi(int lev, int iters) {
...
void MAC2D::mgComputeResidual(int lev) {
    MGLevel& L = mgLevels[lev];
    mgApplyA(lev, L.x, L.Ax);
    const int N = L.nx * L.ny;
    for (int id = 0; id < N; ++id) {
...
void MAC2D::mgRestrictResidual(int fineLev) {
...
void MAC2D::mgProlongateAndAdd(int coarseLev) {
...
void MAC2D::applyMGPrecond(const std::vector<float>& r, std::vector<float>& z) {
    ensureMultigrid();
