...
void pipeBoundaryFieldToSolidMask(const PipeBoundaryField& field,
    ...
    out.assign(n, 0u);
    for (std::size_t i = 0; i < n; ++i) out[i] = (field.wallMask[i] != 0u) ? 1u : 0u;
}
...
void applySolidsToSmoke(MACSmoke3D& smoke, const std::vector<uint8_t>& mask) { smoke.setVoxelSolids(mask); }
void applySolidsToWater(MACWater3D& water, const std::vector<uint8_t>& mask) { water.setVoxelSolids(mask); }
